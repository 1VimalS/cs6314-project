...
app.post('/user', async (request, response) => {
  try {
    ...
    if (!login_name || typeof login_name !== 'string' || !login_name.trim()) {
      return response.status(400).send('login_name is required and must be a non-empty string');
    }

    if (!password || typeof password !== 'string' || !password.trim()) {
      return response.status(400).send('password is required and must be a non-empty string');
    }

    if (!first_name || typeof first_name !== 'string' || !first_name.trim()) {
      return response.status(400).send('first_name is required and must be a non-empty string');
    }

    if (!last_name || typeof last_name !== 'string' || !last_name.trim()) {
      return response.status(400).send('last_name is required and must be a non-empty string');
    }
